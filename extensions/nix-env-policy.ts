...
- On this machine, “available” or “usable” includes tools runnable through a suitable Nix dev shell, not only tools already on the host PATH.
- If a user asks for a tool version or to use a known CLI and it is not on the host PATH, do not stop at “not found”; check for a suitable existing shell or prepare one, then run the CLI there.
...
- If the requested CLI already works directly on the host and the task is a simple one-off command, prefer using the host CLI directly instead of creating an unnecessary shell.
- Shell selection order:
  ...
  - When reusing, prefer the most semantically matching existing shell instead of creating a near-duplicate with a new name.
