...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("latex-renderer-test", {
    ...
    handler: async (_args, ctx) => {
      const markdown = `A clean rendered message hides the formula source and shows the rendered equation below.\n\n$$\n\\int_{-\\infty}^{\\infty} e^{-x^2}\\,dx = \\sqrt{\\pi}\n$$\n\nInline math like $E=mc^2$ is left alone in normal Markdown text.`;
      pi.sendMessage<LatexDetails>({
        customType: CUSTOM_TYPE,
